...
def getSeedURLsFromString(string):
    seeds = string.split()
    return seeds
...
if __name__ == "__main__":
    ...
    if form["crawlField"].value == str(0):
        testBaseFC(form["urlInput"].value, int(form["pageLimit"].value))
    elif form["crawlField"].value == str(1):
        testEventFC(form["urlInput"].value, int(form["pageLimit"].value), form["detailInput"].value)
    else:
        print "Usage: FocusedCrawler.py [0 or 1] [seed location] [page limit]"
        print "0 for baseFC and 1 for eventFC"
    print str(form["id"].value)
    #else:
        #print "Usage: FocusedCrawler.py [0 or 1] [seed location] [page limit]"
        #print "0 for baseFC and 1 for eventFC"
